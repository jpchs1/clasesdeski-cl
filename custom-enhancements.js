...
// La tela no se transforma: se vuelve a dibujar en cada cuadro. Es la
  ...
  var TS_LARGO = [0, .0833, .1667, .25, .3333, .4167, .5,
                  .5833, .6667, .75, .8333, .9167, 1];
  var TS_BLANCO = [.409, .4934, .5779, .6624, .7469, .8314, .9159, 1];
  var TS_AZUL = [0, .0682, .1364, .2045, .2727, .3409, .409];

  // t recorre la tela de 0 (asta) a 1 (extremo libre). Ondea mas cuanto
  // mas lejos del asta, y al ondear la tela se acorta de ancho, que es lo
  // que termina de darle el golpe de viento.
  function ondaBandera(t, fase, amplitud) {
    return amplitud * Math.pow(t, 1.15) * Math.sin(2 * Math.PI * t * 1.6 - fase);
  }

  function anchoBandera(fase, aprieta) {
    return 1 - aprieta * (1 - Math.cos(fase));
  }

  function franjaBandera(ts, arriba, abajo, fase, amplitud, ancho) {
    var d = '', i, x, dy;
    for (i = 0; i < ts.length; i++) {
      x = (9 + 44 * ts[i] * ancho).toFixed(2);
      dy = ondaBandera(ts[i], fase, amplitud);
      d += (i ? 'L' : 'M') + x + ' ' + (arriba + dy).toFixed(2);
    }
    for (i = ts.length - 1; i >= 0; i--) {
      x = (9 + 44 * ts[i] * ancho).toFixed(2);
      dy = ondaBandera(ts[i], fase, amplitud);
      d += 'L' + x + ' ' + (abajo + dy).toFixed(2);
    }
    return d + 'Z';
  }

  function estrellaBandera(fase, amplitud, ancho) {
    var t = 9 / 44;
    var dy = ondaBandera(t, fase, amplitud);
    var dx = 44 * t * (ancho - 1);
    var d = '', i;
    for (i = 0; i < ESTRELLA.length; i++) {
      d += (i ? 'L' : 'M') + (ESTRELLA[i][0] + dx).toFixed(2)
        + ' ' + (ESTRELLA[i][1] + dy).toFixed(2);
    }
    return d + 'Z';
  }
  ...
  function dibujarBandera(raiz) {
    ...
    roja.setAttribute('d', franjaBandera(TS_LARGO, 24, 42, 0, 9, 1));
    raiz.querySelector('.cdski-bandera-blanca').setAttribute('d', franjaBandera(TS_BLANCO, 6, 24, 0, 9, 1));
    raiz.querySelector('.cdski-bandera-azul').setAttribute('d', franjaBandera(TS_AZUL, 6, 24, 0, 9, 1));
    raiz.querySelector('.cdski-bandera-estrella').setAttribute('d', estrellaBandera(0, 9, 1));
  }
...
var HUASO_SVG =
  ...
  function animarEscena(escena) {
    ...
    function cuadro(t) {
      ...
      if (roja) {
        var fase = 2 * Math.PI * ((s / (calma ? 3.6 : 1.05)) % 1);
        var amp = calma ? 4 : 9;
        var anchoT = anchoBandera(fase, calma ? 0.04 : 0.11);
        roja.setAttribute('d', franjaBandera(TS_LARGO, 24, 42, fase, amp, anchoT));
        blanca.setAttribute('d', franjaBandera(TS_BLANCO, 6, 24, fase, amp, anchoT));
        azul.setAttribute('d', franjaBandera(TS_AZUL, 6, 24, fase, amp, anchoT));
        estrella.setAttribute('d', estrellaBandera(fase, amp, anchoT));
      }
