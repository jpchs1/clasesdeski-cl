...
(function($) {
    ...
    function init() {
        ...
        // Unified guardian: keeps enforcing defaults (Dos + Full-day) and
        // capturing prices until BOTH defaults are stable AND prices are non-zero.
        // Runs every 400ms for up to 30 seconds.  This replaces the old separate
        // setTimeout (personas), setInterval (plan) and setInterval (priceWatch).
        var lastPrecio = '';
        var guardianTicks = 0;
        var defaultsStableSince = 0;   // tick at which both defaults first stuck
        var guardian = setInterval(function() {
            ...
            // --- check personas ---
            var personasOk = false;
            f.querySelectorAll('input[name="item_meta[17]"]').forEach(function(r) {
                if (r.value === 'Dos' && r.checked) personasOk = true;
            });
            if (!personasOk) {
                setDefaultPersonas();
                defaultsStableSince = 0;
            }

            // --- check plan ---
            var planOk = false;
            f.querySelectorAll('input[name="item_meta[16]"]').forEach(function(r) {
                if (r.value && r.value.indexOf('Full') > -1 && r.checked) planOk = true;
            });
            if (!planOk) {
                setDefaultPlan();
                defaultsStableSince = 0;
            ...
            // --- track stability ---
            if (personasOk && planOk && defaultsStableSince === 0) {
                defaultsStableSince = guardianTicks;
            }

            // --- capture prices whenever they change ---
            ...
            // Also capture immediately after setting defaults so sidebar updates
            if (!personasOk || !planOk) {
                setTimeout(capturePageOneData, 300);
                setTimeout(capturePageOneData, 800);
            }

            // Stop after defaults have been stable for 5 seconds (12 ticks)
            // OR after 30 seconds total (75 ticks at 400ms)
            var stableTicks = defaultsStableSince > 0 ? (guardianTicks - defaultsStableSince) : 0;
            var pricesLoaded = currentPrecio && parseFloat(currentPrecio) > 0;
            if ((stableTicks >= 12 && pricesLoaded) || guardianTicks >= 75) {
                // One final capture to make sure sidebar is up to date
                capturePageOneData();
