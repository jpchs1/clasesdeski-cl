...
( function( $ ) {
	...
	if (!mounthood_previewer_vars['need_refresh']) {
		var $style = $('#mounthood-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<'+'style type="text/css" id="mounthood-customizer-css"></'+'style'+'>' ).find('#mounthood-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			mounthood_set_cookie(mounthood_previewer_vars['options_prefix']+'_compile_less', 1);
		} );
