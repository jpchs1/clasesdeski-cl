...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.mounthood_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.mounthood_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && mounthood_get_cookie('to_demo_fixed')==1) mounthood_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.mounthood_options_field_info .to_demo_pin', function(e) {
		mounthood_options_fix_demo(jQuery(this).parents('.mounthood_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	mounthood_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.mounthood_options_input,.mounthood_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.mounthood_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		mounthood_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.mounthood_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.mounthood_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .mounthood_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.mounthood_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? MOUNTHOOD_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.mounthood_options_tab').find('> .mounthood_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="mounthood_options_schemes_order" class="mounthood_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function mounthood_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.mounthood_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.mounthood_options').find('>.mounthood_options_field_info');
...
function mounthood_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.mounthood_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	mounthood_message_confirm(MOUNTHOOD_STORAGE['to_strings']['scheme_delete_confirm'], MOUNTHOOD_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'mounthood_options_scheme_delete',
			nonce: MOUNTHOOD_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.mounthood_options_tab').find('> .mounthood_options_schemes_order').val()
		};
		setTimeout(function(){
			mounthood_message_info(MOUNTHOOD_STORAGE['to_strings']['recompile_styles'], MOUNTHOOD_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(MOUNTHOOD_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				mounthood_message_success(MOUNTHOOD_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+MOUNTHOOD_STORAGE['to_strings']['reload_page'], MOUNTHOOD_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				mounthood_message_warning(MOUNTHOOD_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, MOUNTHOOD_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function mounthood_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.mounthood_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	mounthood_message_confirm(MOUNTHOOD_STORAGE['to_strings']['scheme_copy_confirm'], MOUNTHOOD_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'mounthood_options_scheme_copy',
			nonce: MOUNTHOOD_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.mounthood_options_tab').find('> .mounthood_options_schemes_order').val()
		};
		...
		jQuery.post(MOUNTHOOD_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				mounthood_message_success(MOUNTHOOD_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+MOUNTHOOD_STORAGE['to_strings']['reload_page'], MOUNTHOOD_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				mounthood_message_warning(MOUNTHOOD_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, MOUNTHOOD_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function mounthood_options_fix_demo(info) {
    "use strict";
	var info2 = info.parents('.mounthood_options').toggleClass('to_demo_fixed').find('>.mounthood_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.mounthood_options').append(clonedObj);
		info2 = info.parents('.mounthood_options').find('>.mounthood_options_field_info');
	}
	mounthood_set_cookie('to_demo_fixed', jQuery('.mounthood_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function mounthood_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.mounthood_options_field_info').data('scheme');
