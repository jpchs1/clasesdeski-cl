...
function mounthood_theme_customizer(option, val) {
    "use strict";
	var custom_style = '';
	...
	if (option == 'body_style') {
		...
		var params = MOUNTHOOD_STORAGE['co_add_params']!=undefined ? MOUNTHOOD_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#mounthood-customizer-styles-'+option).length > 0 ? jQuery('#mounthood-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="mounthood-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
