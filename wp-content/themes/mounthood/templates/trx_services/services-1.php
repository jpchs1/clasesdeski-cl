...
if ( !function_exists( 'mounthood_template_services_1_theme_setup' ) ) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_template_services_1_theme_setup', 1 );
	function mounthood_template_services_1_theme_setup() {
		mounthood_add_template(array(
			'layout' => 'services-1',
			'template' => 'services-1',
			'mode'   => 'services',
			'title'  => esc_html__('Services /Style 1/', 'mounthood'),
...
// Template output
if ( !function_exists( 'mounthood_template_services_1_output' ) ) {
	function mounthood_template_services_1_output($post_options, $post_data) {
					...
					<div class="sc_services_item_description">
						...
						if ($post_data['post_protected']) {
							...
							if (!empty($post_data['post_link']) && !mounthood_param_is_off($post_options['readmore'])) {
								?><a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_services_item_readmore"><?php mounthood_show_layout($post_options['readmore']); ?><span class="icon-right"></span></a><?php
							}
						}
						?>
					</div>
