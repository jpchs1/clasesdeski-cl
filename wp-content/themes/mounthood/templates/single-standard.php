...
if ( !function_exists( 'mounthood_template_single_standard_theme_setup' ) ) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_template_single_standard_theme_setup', 1 );
	function mounthood_template_single_standard_theme_setup() {
		mounthood_add_template(array(
			'layout' => 'single-standard',
			'mode'   => 'single',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single standard', 'mounthood'),
			'thumb_title'  => esc_html__('Fullwidth image (crop)', 'mounthood'),
			'w'		 => 1170,
			'h'		 => 659
		));
	}
...
// Template output
if ( !function_exists( 'mounthood_template_single_standard_output' ) ) {
	function mounthood_template_single_standard_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] == 'center' && mounthood_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_html($title_tag); ?> itemprop="<?php echo (float) $avg_author > 0 || (float) $avg_users > 0 ? 'itemReviewed' : 'headline'; ?>" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php mounthood_show_layout($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
		<?php
		}


        if ($show_title && $post_options['location'] != 'center' && !empty($post_data['post_title'])) {
            ?><h3 class="post_title_single"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php mounthood_show_layout($post_data['post_title']); ?></a></h3><?php
        }
		...
		if (!$post_data['post_protected'] && (
			...
			(mounthood_get_custom_option('show_featured_image')=='yes' && $post_data['post_thumb'])	// && $post_data['post_format']!='gallery' && $post_data['post_format']!='image')
		)) {
			?>
		...
		$sidebar_present = !mounthood_param_is_off(mounthood_get_custom_option('show_sidebar_main'));
		if (!$sidebar_present) mounthood_close_wrapper();
