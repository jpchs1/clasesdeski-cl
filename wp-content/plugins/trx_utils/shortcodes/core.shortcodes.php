<?php
/**
 * Mounthood Framework: shortcodes manipulations
 *
 * @package	mounthood
 * @since	mounthood 1.0
 */
...
// Theme init
if (!function_exists('mounthood_sc_theme_setup')) {
	add_action( 'mounthood_action_init_theme', 'mounthood_sc_theme_setup', 1 );
	function mounthood_sc_theme_setup() {
		// Add sc stylesheets
		add_action('mounthood_action_add_styles', 'mounthood_sc_add_styles', 1);
	}
}

if (!function_exists('mounthood_sc_theme_setup2')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_sc_theme_setup2' );
	function mounthood_sc_theme_setup2() {

		if ( !is_admin() || isset($_POST['action']) ) {
			// Enable/disable shortcodes in excerpt
			add_filter('the_excerpt', 					'mounthood_sc_excerpt_shortcodes');
	
			// Prepare shortcodes in the content
			if (function_exists('mounthood_sc_prepare_content')) mounthood_sc_prepare_content();
		}

		// Add init script into shortcodes output in VC frontend editor
		add_filter('mounthood_shortcode_output', 'mounthood_sc_add_scripts', 10, 4);

		// AJAX: Send contact form data
		add_action('wp_ajax_send_form',			'mounthood_sc_form_send');
		add_action('wp_ajax_nopriv_send_form',	'mounthood_sc_form_send');

		// Show shortcodes list in admin editor
		add_action('media_buttons',				'mounthood_sc_selector_add_in_toolbar', 11);
...
if ( !function_exists( 'mounthood_sc_add_styles' ) ) {
	//add_action('mounthood_action_add_styles', 'mounthood_sc_add_styles', 1);
	function mounthood_sc_add_styles() {
		// Shortcodes
		wp_enqueue_style( 'mounthood-shortcodes-style',	trx_utils_get_file_url('shortcodes/theme.shortcodes.css'), array(), null );
	}
}


// Register shortcodes init scripts
if ( !function_exists( 'mounthood_sc_add_scripts' ) ) {
	//add_filter('mounthood_shortcode_output', 'mounthood_sc_add_scripts', 10, 4);
	function mounthood_sc_add_scripts($output, $tag='', $atts=array(), $content='') {

		if (mounthood_storage_empty('shortcodes_scripts_added')) {
			mounthood_storage_set('shortcodes_scripts_added', true);
			wp_enqueue_script( 'mounthood-shortcodes-script', trx_utils_get_file_url('shortcodes/theme.shortcodes.js'), array('jquery'), null, true );
		}
...
if (!function_exists('mounthood_sc_prepare_content')) {
	function mounthood_sc_prepare_content() {
		if (function_exists('mounthood_sc_clear_around')) {
			$filters = array(
				array('mounthood', 'sc', 'clear', 'around'),
				array('widget', 'text'),
				array('the', 'excerpt'),
				array('the', 'content')
			);
			...
			if (is_array($filters) && count($filters) > 0) {
				foreach ($filters as $flt)
					add_filter(join('_', $flt), 'mounthood_sc_clear_around', 1);	// Priority 1 to clear spaces before do_shortcodes()
			}
		}
	}
}
...
if (!function_exists('mounthood_sc_excerpt_shortcodes')) {
	//add_filter('the_excerpt', 'mounthood_sc_excerpt_shortcodes');
	function mounthood_sc_excerpt_shortcodes($content) {
		if (!empty($content)) {
...
*/
if (!function_exists('mounthood_sc_clear_around')) {
	function mounthood_sc_clear_around($content) {
...
// Mounthood shortcodes load scripts
if (!function_exists('mounthood_sc_load_scripts')) {
	function mounthood_sc_load_scripts() {
		static $loaded = false;
		if (!$loaded) {
			wp_enqueue_script( 'mounthood-shortcodes_admin-script', trx_utils_get_file_url('shortcodes/shortcodes_admin.js'), array('jquery'), null, true );
			wp_enqueue_script( 'mounthood-selection-script',  mounthood_get_file_url('js/jquery.selection.js'), array('jquery'), null, true );
			wp_localize_script( 'mounthood-shortcodes_admin-script', 'MOUNTHOOD_SHORTCODES_DATA', mounthood_storage_get('shortcodes') );
			$loaded = true;
		}
	}
}

// Mounthood shortcodes prepare scripts
if (!function_exists('mounthood_sc_prepare_scripts')) {
	function mounthood_sc_prepare_scripts() {
		static $prepared = false;
		if (!$prepared) {
			mounthood_storage_set_array('js_vars', 'shortcodes_cp', is_admin() ? (!mounthood_storage_empty('to_colorpicker') ? mounthood_storage_get('to_colorpicker') : 'wp') : 'custom');	// wp | tiny | custom
			$prepared = true;
		}
	}
}
...
// Show shortcodes list in admin editor
if (!function_exists('mounthood_sc_selector_add_in_toolbar')) {
	//add_action('media_buttons','mounthood_sc_selector_add_in_toolbar', 11);
	function mounthood_sc_selector_add_in_toolbar(){

		if ( !mounthood_options_is_used() ) return;

		mounthood_sc_load_scripts();
		mounthood_sc_prepare_scripts();

		$shortcodes = mounthood_storage_get('shortcodes');
		...
		mounthood_show_layout($shortcodes_list);
	}
}

// Mounthood shortcodes builder settings
...
// Mounthood shortcodes implementation
