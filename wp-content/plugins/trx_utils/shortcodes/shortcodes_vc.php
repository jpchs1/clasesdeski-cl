...
if ( !function_exists( 'mounthood_vc_width' ) ) {
	function mounthood_vc_width($w='') {
		return array(
			"param_name" => "width",
...
if ( !function_exists( 'mounthood_vc_height' ) ) {
	function mounthood_vc_height($h='') {
		return array(
			"param_name" => "height",
...
if ( !function_exists( 'mounthood_shortcodes_vc_scripts_admin' ) ) {
	//add_action( 'admin_enqueue_scripts', 'mounthood_shortcodes_vc_scripts_admin' );
	function mounthood_shortcodes_vc_scripts_admin() {
		// Include CSS 
...
// Load scripts and styles for VC support
if ( !function_exists( 'mounthood_shortcodes_vc_scripts_front' ) ) {
	//add_action( 'wp_enqueue_scripts', 'mounthood_shortcodes_vc_scripts_front' );
	function mounthood_shortcodes_vc_scripts_front() {
...
if ( !function_exists( 'mounthood_shortcodes_vc_add_init_script' ) ) {
	//add_filter('mounthood_shortcode_output', 'mounthood_shortcodes_vc_add_init_script', 10, 4);
	function mounthood_shortcodes_vc_add_init_script($output, $tag='', $atts=array(), $content='') {
		if ( (isset($_GET['vc_editable']) && $_GET['vc_editable']=='true') && (isset($_POST['action']) && $_POST['action']=='vc_load_shortcode')
				&& ( isset($_POST['shortcodes'][0]['tag']) && $_POST['shortcodes'][0]['tag']==$tag )
		) {
			if (mounthood_strpos($output, 'mounthood_vc_init_shortcodes')===false) {
				$id = "mounthood_vc_init_shortcodes_".str_replace('.', '', mt_rand());
				...
				$output .= '<'.trim($holder).' id="'.esc_attr($id).'">
						try {
							...
							mounthood_init_shortcodes(jQuery("body").eq(0));
...
if ( !function_exists( 'mounthood_get_vc_param' ) ) {
	function mounthood_get_vc_param($prm) {
		return mounthood_storage_get_array('vc_params', $prm);
	}
}

// Set vc_param value
if ( !function_exists( 'mounthood_set_vc_param' ) ) {
	function mounthood_set_vc_param($prm, $val) {
		mounthood_storage_set_array('vc_params', $prm, $val);
	}
}
...
if ( !function_exists( 'mounthood_shortcodes_vc_theme_setup' ) ) {
	//if ( mounthood_vc_is_frontend() )
	if ( (isset($_GET['vc_editable']) && $_GET['vc_editable']=='true') || (isset($_GET['vc_action']) && $_GET['vc_action']=='vc_inline') )
		add_action( 'mounthood_action_before_init_theme', 'mounthood_shortcodes_vc_theme_setup', 20 );
	else
		add_action( 'mounthood_action_after_init_theme', 'mounthood_shortcodes_vc_theme_setup' );
	function mounthood_shortcodes_vc_theme_setup() {
		...
		$scheme = array(
					...
					"class" => "",
					"value" => array_flip(mounthood_get_list_color_schemes(true)),
					"type" => "dropdown"
		);
		...
		add_filter( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'mounthood_shortcodes_vc_add_params_classes', 10, 3 );

		if (mounthood_shortcodes_is_used() && class_exists('MOUNTHOOD_VC_ShortCodeSingle')) {
			...
			mounthood_storage_set('vc_params', array(
				
				// Common arrays and strings
				'category' => esc_html__("Mounthood shortcodes", 'trx_utils'),
				...
				'animation' => array(
					...
					"group" => esc_html__('ID &amp; Class', 'trx_utils'),
					"class" => "",
					"value" => array_flip(mounthood_get_sc_param('animations')),
					"type" => "dropdown"
				),
				...
				'margin_top' => array(
					...
					"group" => esc_html__('Size &amp; Margins', 'trx_utils'),
					"std" => "inherit",
					"value" => array_flip(mounthood_get_sc_param('margins')),
					"type" => "dropdown"
				),
			...
			do_action('mounthood_action_shortcodes_list_vc');
...
if ( !function_exists( 'mounthood_shortcodes_vc_add_params_classes' ) ) {
	//Handler of the add_filter( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG,				'mounthood_shortcodes_vc_add_params_classes', 10, 3 );
	function mounthood_shortcodes_vc_add_params_classes($classes, $sc, $atts) {
		if (in_array($sc, array('vc_row', 'vc_row_inner', 'vc_column', 'vc_column_inner', 'vc_column_text'))) {
			if (!empty($atts['scheme']) && !mounthood_param_is_off($atts['scheme']) && !mounthood_param_is_inherit($atts['scheme']))
				$classes .= ($classes ? ' ' : '') . 'scheme_' . $atts['scheme'];
		}
		if (in_array($sc, array('vc_row'))) {
			if (!empty($atts['inverse']) && !mounthood_param_is_off($atts['inverse']))
