...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('mounthood_sc_icon_theme_setup')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_sc_icon_theme_setup' );
	function mounthood_sc_icon_theme_setup() {
		add_action('mounthood_action_shortcodes_list', 		'mounthood_sc_icon_reg_shortcodes');
		if (function_exists('mounthood_exists_visual_composer') && mounthood_exists_visual_composer())
			add_action('mounthood_action_shortcodes_list_vc','mounthood_sc_icon_reg_shortcodes_vc');
	}
}
...
if (!function_exists('mounthood_sc_icon')) {	
	function mounthood_sc_icon($atts, $content=null){	
		...
		$css2 = ($font_weight != '' && !mounthood_is_inherit_option($font_weight) ? 'font-weight:'. esc_attr($font_weight).';' : '')
			. ($font_size != '' ? 'font-size:' . esc_attr(mounthood_prepare_css_value($font_size)) . '; line-height: ' . (!$bg_shape || mounthood_param_is_inherit($bg_shape) ? '1' : '1.2') . 'em;' : '')
		...
		$output = $icon!='' 
			? ($link ? '<a href="'.esc_url($link).'"' : '<span') . ($id ? ' id="'.esc_attr($id).'"' : '')
				. ' class="sc_icon '.esc_attr($icon)
					. ($bg_shape && !mounthood_param_is_inherit($bg_shape) ? ' sc_icon_shape_'.esc_attr($bg_shape) : '')
				...
				.($link ? '</a>' : '</span>')
			: '';
		return apply_filters('mounthood_shortcode_output', $output, 'trx_icon', $atts, $content);
	}
	mounthood_require_shortcode('trx_icon', 'mounthood_sc_icon');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_icon_reg_shortcodes' ) ) {
	//add_action('mounthood_action_shortcodes_list', 'mounthood_sc_icon_reg_shortcodes');
	function mounthood_sc_icon_reg_shortcodes() {
	
		mounthood_sc_map("trx_icon", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_icon_reg_shortcodes_vc' ) ) {
	//add_action('mounthood_action_shortcodes_list_vc', 'mounthood_sc_icon_reg_shortcodes_vc');
	function mounthood_sc_icon_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Icon extends MOUNTHOOD_VC_ShortCodeSingle {}
	}
}
?>