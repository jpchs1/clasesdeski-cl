...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('mounthood_sc_button_theme_setup')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_sc_button_theme_setup' );
	function mounthood_sc_button_theme_setup() {
		add_action('mounthood_action_shortcodes_list', 		'mounthood_sc_button_reg_shortcodes');
		if (function_exists('mounthood_exists_visual_composer') && mounthood_exists_visual_composer())
			add_action('mounthood_action_shortcodes_list_vc','mounthood_sc_button_reg_shortcodes_vc');
	}
}
...
if (!function_exists('mounthood_sc_button')) {	
	function mounthood_sc_button($atts, $content=null){	
		...
		if (mounthood_param_is_on($popup)) mounthood_enqueue_popup('magnific');
		$output = '<a href="' . (empty($link) ? '#' : $link) . '"'
			...
			. ' class="sc_button sc_button_' . esc_attr($type) 
					...
					. (mounthood_param_is_on($popup) ? ' sc_popup_link' : '') 
					. '"'
			. ($id ? ' id="'.esc_attr($id).'"' : '') 
			...
			. '</a>';
		return apply_filters('mounthood_shortcode_output', $output, 'trx_button', $atts, $content);
	}
	mounthood_require_shortcode('trx_button', 'mounthood_sc_button');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_button_reg_shortcodes' ) ) {
	//add_action('mounthood_action_shortcodes_list', 'mounthood_sc_button_reg_shortcodes');
	function mounthood_sc_button_reg_shortcodes() {
	
		mounthood_sc_map("trx_button", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"options" => array(
						...
						'simple' => esc_html__('Simple', 'trx_utils'),
						'simple_alt_hover' => esc_html__('Simple Alt Hover', 'trx_utils'),
						'simple_white' => esc_html__('Simple White', 'trx_utils')
					),
					"type" => "checklist"
				), 
				"size" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_button_reg_shortcodes_vc' ) ) {
	//add_action('mounthood_action_shortcodes_list_vc', 'mounthood_sc_button_reg_shortcodes_vc');
	function mounthood_sc_button_reg_shortcodes_vc() {
			...
			"params" => array(
				...
				array(
					...
					"value" => array(
						...
						esc_html__('Simple', 'trx_utils') => 'simple',
						esc_html__('Simple Alter', 'trx_utils') => 'simple_alter',
						esc_html__('Simple Alt Hover', 'trx_utils') => 'simple_alt_hover',
						esc_html__('Simple White', 'trx_utils') => 'simple_white'
					),
					"type" => "dropdown"
				),
				array(
		...
		class WPBakeryShortCode_Trx_Button extends MOUNTHOOD_VC_ShortCodeSingle {}
	}
}
?>