...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('mounthood_sc_tabs_theme_setup')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_sc_tabs_theme_setup' );
	function mounthood_sc_tabs_theme_setup() {
		add_action('mounthood_action_shortcodes_list', 		'mounthood_sc_tabs_reg_shortcodes');
		if (function_exists('mounthood_exists_visual_composer') && mounthood_exists_visual_composer())
			add_action('mounthood_action_shortcodes_list_vc','mounthood_sc_tabs_reg_shortcodes_vc');
	}
}
...
if (!function_exists('mounthood_sc_tabs')) {	
	function mounthood_sc_tabs($atts, $content = null) {
		...
		mounthood_storage_set('sc_tab_data', array(
			'counter'=> 0,
            'scroll' => $scroll,
		...
		$sc_tab_titles = mounthood_storage_get_array('sc_tab_data', 'titles');
		...
		$tabs_output .= $titles_output
			...
			. ($content)
			.'</div>';
		return apply_filters('mounthood_shortcode_output', $tabs_output, 'trx_tabs', $atts, $content);
	}
	mounthood_require_shortcode("trx_tabs", "mounthood_sc_tabs");
}


if (!function_exists('mounthood_sc_tab')) {	
	function mounthood_sc_tab($atts, $content = null) {
		if (mounthood_in_shortcode_blogger()) return '';
		extract(mounthood_html_decode(shortcode_atts(array(
			// Individual params
		...
		mounthood_storage_inc_array('sc_tab_data', 'counter');
		$counter = mounthood_storage_get_array('sc_tab_data', 'counter');
		if (empty($id))
			$id = !empty($tab_id) ? $tab_id : mounthood_storage_get_array('sc_tab_data', 'id').'_'.intval($counter);
		$sc_tab_titles = mounthood_storage_get_array('sc_tab_data', 'titles');
		...
		mounthood_storage_set_array('sc_tab_data', 'titles', $sc_tab_titles);
		$output = '<div id="'.esc_attr($id).'"'
					.' class="sc_tabs_content' 
						...
						. ($css!='' ? ' style="'.esc_attr($css).'"' : '') 
						. '>' 
				. (mounthood_param_is_on(mounthood_storage_get_array('sc_tab_data', 'scroll')) 
					? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.(mounthood_storage_get_array('sc_tab_data', 'height') != '' ? mounthood_storage_get_array('sc_tab_data', 'height') : '200px').';"><div class="sc_scroll_wrapper swiper-wrapper"><div class="sc_scroll_slide swiper-slide">' 
					: '')
				...
				. (mounthood_param_is_on(mounthood_storage_get_array('sc_tab_data', 'scroll')) 
					...
					: '')
			. '</div>';
		return apply_filters('mounthood_shortcode_output', $output, 'trx_tab', $atts, $content);
	}
	mounthood_require_shortcode("trx_tab", "mounthood_sc_tab");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_tabs_reg_shortcodes' ) ) {
	//add_action('mounthood_action_shortcodes_list', 'mounthood_sc_tabs_reg_shortcodes');
	function mounthood_sc_tabs_reg_shortcodes() {
	
		mounthood_sc_map("trx_tabs", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_tabs_reg_shortcodes_vc' ) ) {
	//add_action('mounthood_action_shortcodes_list_vc', 'mounthood_sc_tabs_reg_shortcodes_vc');
	function mounthood_sc_tabs_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Tabs extends MOUNTHOOD_VC_ShortCodeTabs {}
		class WPBakeryShortCode_Trx_Tab extends MOUNTHOOD_VC_ShortCodeTab {}
	}
}
?>