...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('mounthood_sc_skills_theme_setup')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_sc_skills_theme_setup' );
	function mounthood_sc_skills_theme_setup() {
		add_action('mounthood_action_shortcodes_list', 		'mounthood_sc_skills_reg_shortcodes');
		if (function_exists('mounthood_exists_visual_composer') && mounthood_exists_visual_composer())
			add_action('mounthood_action_shortcodes_list_vc','mounthood_sc_skills_reg_shortcodes_vc');
	}
}
...
if (!function_exists('mounthood_sc_skills')) {	
	function mounthood_sc_skills($atts, $content=null){	
		...
		mounthood_storage_set('sc_skills_data', array(
			'counter' => 0,
            'columns' => 0,
            'height'  => 0,
            'type'    => $type,
            'pie_compact' => mounthood_param_is_on($pie_compact) ? 'on' : 'off',
		...
		mounthood_enqueue_diagram($type);
		if ($type!='arc') {
			if ($layout=='' || ($layout=='columns' && $columns<1)) $layout = 'rows';
			if ($layout=='columns') mounthood_storage_set_array('sc_skills_data', 'columns', $columns);
			if ($type=='bar') {
		...
		if ($style) {
			...
			mounthood_storage_set_array('sc_skills_data', 'style', $style);
		}
		mounthood_storage_set_array('sc_skills_data', 'max', $max_value);
		mounthood_storage_set_array('sc_skills_data', 'dir', $dir);
		mounthood_storage_set_array('sc_skills_data', 'height', mounthood_prepare_css_value($height));
		...
		if (!mounthood_storage_empty('sc_skills_data', 'height') && (mounthood_storage_get_array('sc_skills_data', 'type') == 'arc' || (mounthood_storage_get_array('sc_skills_data', 'type') == 'pie' && mounthood_param_is_on(mounthood_storage_get_array('sc_skills_data', 'pie_compact')))))
			$css .= 'height: '.mounthood_storage_get_array('sc_skills_data', 'height');
		$content = do_shortcode($content);
		$output = '<div id="'.esc_attr($id).'"' 
					. ' class="sc_skills sc_skills_' . esc_attr($type) 
						. ($type=='bar' ? ' sc_skills_'.esc_attr($dir) : '') 
						. ($type=='pie' ? ' sc_skills_compact_'.esc_attr(mounthood_storage_get_array('sc_skills_data', 'pie_compact')) : '') 
				...
				. '>'
					...
					. ($type=='arc' 
						? ('<div class="sc_skills_legend">'.(mounthood_storage_get_array('sc_skills_data', 'legend')).'</div>'
							. '<div id="'.esc_attr($id).'_diagram" class="sc_skills_arc_canvas"></div>'
							. '<div class="sc_skills_data" style="display:none;">' . (mounthood_storage_get_array('sc_skills_data', 'data')) . '</div>'
						  )
						: '')
					. ($type=='pie' && mounthood_param_is_on(mounthood_storage_get_array('sc_skills_data', 'pie_compact'))
						? ('<div class="sc_skills_legend">'.(mounthood_storage_get_array('sc_skills_data', 'legend')).'</div>'
				...
				. '</div>';
		return apply_filters('mounthood_shortcode_output', $output, 'trx_skills', $atts, $content);
	}
	mounthood_require_shortcode('trx_skills', 'mounthood_sc_skills');
}


if (!function_exists('mounthood_sc_skills_item')) {	
	function mounthood_sc_skills_item($atts, $content=null) {
		if (mounthood_in_shortcode_blogger()) return '';
		extract(mounthood_html_decode(shortcode_atts( array(
			// Individual params
			"title" => "",
		...
		mounthood_storage_inc_array('sc_skills_data', 'counter');
		$ed = mounthood_substr($value, -1)=='%' ? '%' : '';
		$value = str_replace('%', '', $value);
		$value = intval($value);
		if (mounthood_storage_get_array('sc_skills_data', 'max') < $value) mounthood_storage_set_array('sc_skills_data', 'max', $value);
		$percent = round($value / mounthood_storage_get_array('sc_skills_data', 'max') * 100);
		$start = 0;
		$stop = $value;
		$steps = 100;
		$step = max(1, round(mounthood_storage_get_array('sc_skills_data', 'max')/$steps));
		$speed = mt_rand(10,40);
		...
		$title_block = '<div class="sc_skills_info"><div class="sc_skills_label">' . ($title) . '</div>' 
			...
			. '</div>';
		$old_color = $color;
		if (empty($color)) $color = mounthood_storage_get_array('sc_skills_data', 'color');
		if (empty($color)) $color = mounthood_get_scheme_color('text_link', $color);
		if (empty($bg_color)) $bg_color = mounthood_storage_get_array('sc_skills_data', 'bg_color');
		if (empty($bg_color)) $bg_color = mounthood_get_scheme_color('bg_color', $bg_color);
		if (empty($border_color)) $border_color = mounthood_storage_get_array('sc_skills_data', 'border_color');
		if (empty($border_color)) $border_color = mounthood_get_scheme_color('bd_color', $border_color);;
		if (empty($style)) $style = mounthood_storage_get_array('sc_skills_data', 'style');
		$style = max(1, min(4, $style));
		$output = '';
		if (mounthood_storage_get_array('sc_skills_data', 'type') == 'arc' || (mounthood_storage_get_array('sc_skills_data', 'type') == 'pie' && mounthood_param_is_on(mounthood_storage_get_array('sc_skills_data', 'pie_compact')))) {
			if (mounthood_storage_get_array('sc_skills_data', 'type') == 'arc' && empty($old_color)) {
				$rgb = mounthood_hex2rgb($color);
				$color = 'rgba('.(int)$rgb['r'].','.(int)$rgb['g'].','.(int)$rgb['b'].','.(1 - 0.1*(mounthood_storage_get_array('sc_skills_data', 'counter')-1)).')';
			}
			mounthood_storage_concat_array('sc_skills_data', 'legend', 
			...
			mounthood_storage_concat_array('sc_skills_data', 'data', 
				'<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
					. ' class="'.esc_attr(mounthood_storage_get_array('sc_skills_data', 'type')).'"'
					. (mounthood_storage_get_array('sc_skills_data', 'type')=='pie'
						? ( ' data-start="'.esc_attr($start).'"'
							. ' data-stop="'.esc_attr($stop).'"'
							. ' data-step="'.esc_attr($step).'"'
							. ' data-steps="'.esc_attr($steps).'"'
							. ' data-max="'.esc_attr(mounthood_storage_get_array('sc_skills_data', 'max')).'"'
							...
							. ' data-cutout="'.esc_attr(mounthood_storage_get_array('sc_skills_data', 'pie_cutout')).'"'
			...
			$output .= (mounthood_storage_get_array('sc_skills_data', 'columns') > 0 
							? '<div class="sc_skills_column column-1_'.esc_attr(mounthood_storage_get_array('sc_skills_data', 'columns')).'">' 
							: '')
					. (mounthood_storage_get_array('sc_skills_data', 'type')=='bar' && mounthood_storage_get_array('sc_skills_data', 'dir')=='horizontal' ? $title_block : '')
					. '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
						. ' class="sc_skills_item' . ($style ? ' sc_skills_style_'.esc_attr($style) : '') 
							. (!empty($class) ? ' '.esc_attr($class) : '')
							. (mounthood_storage_get_array('sc_skills_data', 'counter') % 2 == 1 ? ' odd' : ' even') 
							. (mounthood_storage_get_array('sc_skills_data', 'counter') == 1 ? ' first' : '') 
							. '"'
						. (mounthood_storage_get_array('sc_skills_data', 'height') !='' || $css 
							? ' style="' 
								. (mounthood_storage_get_array('sc_skills_data', 'height') !='' 
										? 'height: '.esc_attr(mounthood_storage_get_array('sc_skills_data', 'height')).';' 
										: '') 
								. ($css) 
								. '"' 
							: '')
			...
			if (in_array(mounthood_storage_get_array('sc_skills_data', 'type'), array('bar', 'counter'))) {
				// $output .= '<div class="sc_skills_count"' . (mounthood_storage_get_array('sc_skills_data', 'type')=='bar' && $color ? ' style="background-color:' . esc_attr($color) . '; border-color:' . esc_attr($color) . '"' : '') . '>'
				...
				$output .= '<div class="sc_skills_total"'
								...
								. ($start) . ($ed)
							.'</div>'
						.'<div class="sc_skills_count"' . (mounthood_storage_get_array('sc_skills_data', 'type')=='bar' && $color ? ' style="background-color:' . esc_attr($color) . '; border-color:' . esc_attr($color) . '"' : '') . '>'
						. '</div>';
			} else if (mounthood_storage_get_array('sc_skills_data', 'type')=='pie') {
			...
			$output .= 
					  (mounthood_storage_get_array('sc_skills_data', 'type')=='counter' ? $title_block : '')
					. '</div>'
					. (mounthood_storage_get_array('sc_skills_data', 'type')=='bar' && mounthood_storage_get_array('sc_skills_data', 'dir')=='vertical' || mounthood_storage_get_array('sc_skills_data', 'type') == 'pie' ? $title_block : '')
					. (mounthood_storage_get_array('sc_skills_data', 'columns') > 0 ? '</div>' : '');
		}
		return apply_filters('mounthood_shortcode_output', $output, 'trx_skills_item', $atts, $content);
	}
	mounthood_require_shortcode('trx_skills_item', 'mounthood_sc_skills_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_skills_reg_shortcodes' ) ) {
	//add_action('mounthood_action_shortcodes_list', 'mounthood_sc_skills_reg_shortcodes');
	function mounthood_sc_skills_reg_shortcodes() {
	
		mounthood_sc_map("trx_skills", array(
			...
			"params" => array(
				...
				"dir" => array(
					...
					"type" => "checklist",
					"dir" => "horizontal",
					"options" => mounthood_get_sc_param('dir')
				), 
				"style" => array(
			...
			"children" => array(
				...
				"params" => array(
					...
					"addinfo" => array(
						"title" => esc_html__("Additional info", 'trx_utils'),
						"desc" => wp_kses_data( __("Additional info for current skills item", 'trx_utils') ),
						"value" => "",
						"type" => "text"
					),
					"value" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_skills_reg_shortcodes_vc' ) ) {
	//add_action('mounthood_action_shortcodes_list_vc', 'mounthood_sc_skills_reg_shortcodes_vc');
	function mounthood_sc_skills_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				...
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(mounthood_get_sc_param('dir')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "style",
		...
		vc_map( array(
			...
			"params" => array(
				...
				array(
					"param_name" => "addinfo",
					"heading" => esc_html__("Additional info", 'trx_utils'),
					"description" => wp_kses_data( __("Additional info for current skills item", 'trx_utils') ),
					"admin_label" => true,
					"class" => "",
					"value" => "",
					"type" => "textfield"
		...
		class WPBakeryShortCode_Trx_Skills extends MOUNTHOOD_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Skills_Item extends MOUNTHOOD_VC_ShortCodeSingle {}
	}
}
?>