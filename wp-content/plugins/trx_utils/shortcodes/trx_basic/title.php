...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('mounthood_sc_title_theme_setup')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_sc_title_theme_setup' );
	function mounthood_sc_title_theme_setup() {
		add_action('mounthood_action_shortcodes_list', 		'mounthood_sc_title_reg_shortcodes');
		if (function_exists('mounthood_exists_visual_composer') && mounthood_exists_visual_composer())
			add_action('mounthood_action_shortcodes_list_vc','mounthood_sc_title_reg_shortcodes_vc');
	}
}
...
if (!function_exists('mounthood_sc_title')) {	
	function mounthood_sc_title($atts, $content=null){	
		...
		$css .= mounthood_get_css_dimensions_from_values($width)
			.($align && $align!='none' && !mounthood_param_is_inherit($align) ? 'text-align:' . esc_attr($align) .';' : '')
		...
		$pic = $style!='iconed' 
			...
			: '<span class="sc_title_icon sc_title_icon_'.esc_attr($position).'  sc_title_icon_'.esc_attr($image_size).($icon!='' && $icon!='none' ? ' '.esc_attr($icon) : '').'"'.'>'
				...
				.(empty($picture) && $image && $image!='none' ? '<img src="'.esc_url(mounthood_strpos($image, 'http')===0 ? $image : mounthood_get_file_url('images/icons/'.($image).'.png')).'" alt="" />' : '')
				.'</span>';
		$output = '<h' . esc_attr($type) . ($id ? ' id="'.esc_attr($id).'"' : '')
				. ' class="sc_title sc_title_'.esc_attr($style)
					.($align && $align!='none' && !mounthood_param_is_inherit($align) ? ' sc_align_' . esc_attr($align) : '')
				...
				. '</h' . esc_attr($type) . '>';
		return apply_filters('mounthood_shortcode_output', $output, 'trx_title', $atts, $content);
	}
	mounthood_require_shortcode('trx_title', 'mounthood_sc_title');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_title_reg_shortcodes' ) ) {
	//add_action('mounthood_action_shortcodes_list', 'mounthood_sc_title_reg_shortcodes');
	function mounthood_sc_title_reg_shortcodes() {
	
		mounthood_sc_map("trx_title", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"options" => array(
						...
						'underline_dark' => esc_html__('Underline Dark', 'trx_utils'),
						'underline_white' => esc_html__('Underline White', 'trx_utils'),
				...
				"image" => array(
					...
					"type" => "images",
					"size" => "small",
					"options" => mounthood_get_sc_param('images')
				),
				"picture" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_title_reg_shortcodes_vc' ) ) {
	//add_action('mounthood_action_shortcodes_list_vc', 'mounthood_sc_title_reg_shortcodes_vc');
	function mounthood_sc_title_reg_shortcodes_vc() {
			...
			"params" => array(
				...
				array(
					...
					"value" => array(
						...
						esc_html__('Underline Dark', 'trx_utils') => 'underline_dark',
						esc_html__('Underline White', 'trx_utils') => 'underline_white',
				...
				array(
					...
					"value" => mounthood_get_sc_param('images'),
					"type" => "dropdown"
				),
				array(
					"param_name" => "picture",
		...
		class WPBakeryShortCode_Trx_Title extends MOUNTHOOD_VC_ShortCodeSingle {}
	}
}
?>