...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('mounthood_sc_googlemap_theme_setup')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_sc_googlemap_theme_setup' );
	function mounthood_sc_googlemap_theme_setup() {
		add_action('mounthood_action_shortcodes_list', 		'mounthood_sc_googlemap_reg_shortcodes');
		if (function_exists('mounthood_exists_visual_composer') && mounthood_exists_visual_composer())
			add_action('mounthood_action_shortcodes_list_vc','mounthood_sc_googlemap_reg_shortcodes_vc');
	}
}
...
if (!function_exists('mounthood_sc_googlemap')) {	
	function mounthood_sc_googlemap($atts, $content = null) {
		...
		if (empty($style)) $style = mounthood_get_custom_option('googlemap_style');
		...
		mounthood_storage_set('sc_googlemap_markers', array());
		$content = do_shortcode($content);
		$output = '';
		$markers = mounthood_storage_get('sc_googlemap_markers');
		if (count($markers) == 0) {
			$markers[] = array(
				'title' => mounthood_get_custom_option('googlemap_title'),
				'description' => mounthood_strmacros(mounthood_get_custom_option('googlemap_description')),
				'latlng' => mounthood_get_custom_option('googlemap_latlng'),
				'address' => mounthood_get_custom_option('googlemap_address'),
				'point' => mounthood_get_custom_option('googlemap_marker')
			);
		}
		$output .= 
        ...
        foreach ($markers as $marker) {
            ...
            if (mounthood_get_theme_option('api_google') != '') {
                $output .= '<div id="'.esc_attr($marker['id']).'" class="sc_googlemap_marker"'
                    . ' data-title="'.esc_attr($marker['title']).'"'
                    . ' data-description="'.esc_attr(mounthood_strmacros($marker['description'])).'"'
		...
		return apply_filters('mounthood_shortcode_output', $output, 'trx_googlemap', $atts, $content);
	}
	mounthood_require_shortcode("trx_googlemap", "mounthood_sc_googlemap");
}


if (!function_exists('mounthood_sc_googlemap_marker')) {	
	function mounthood_sc_googlemap_marker($atts, $content = null) {
		if (mounthood_in_shortcode_blogger()) return '';
		extract(mounthood_html_decode(shortcode_atts(array(
			// Individual params
			"title" => "",
		...
		$content = do_shortcode($content);
		mounthood_storage_set_array('sc_googlemap_markers', '', array(
			'id' => $id,
			'title' => $title,
			'description' => !empty($content) ? $content : $address,
			'latlng' => $latlng,
			'address' => $address,
			'point' => $point ? $point : mounthood_get_custom_option('googlemap_marker')
			)
		);
		return '';
	}
	mounthood_require_shortcode("trx_googlemap_marker", "mounthood_sc_googlemap_marker");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_googlemap_reg_shortcodes' ) ) {
	//add_action('mounthood_action_shortcodes_list', 'mounthood_sc_googlemap_reg_shortcodes');
	function mounthood_sc_googlemap_reg_shortcodes() {
	
		mounthood_sc_map("trx_googlemap", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"value" => "default",
					"type" => "checklist",
					"options" => mounthood_get_sc_param('googlemap_styles')
				),
				"scheme" => array(
				...
				"width" => mounthood_shortcodes_width('100%'),
				"height" => mounthood_shortcodes_height(240),
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_googlemap_reg_shortcodes_vc' ) ) {
	//add_action('mounthood_action_shortcodes_list_vc', 'mounthood_sc_googlemap_reg_shortcodes_vc');
	function mounthood_sc_googlemap_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				...
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(mounthood_get_sc_param('googlemap_styles')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scheme",
				...
				mounthood_vc_width('100%'),
				mounthood_vc_height(240),
		...
		class WPBakeryShortCode_Trx_Googlemap extends MOUNTHOOD_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Googlemap_Marker extends MOUNTHOOD_VC_ShortCodeCollection {}
	}
}
?>