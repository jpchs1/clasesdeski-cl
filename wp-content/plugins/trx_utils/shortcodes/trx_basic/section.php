...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('mounthood_sc_section_theme_setup')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_sc_section_theme_setup' );
	function mounthood_sc_section_theme_setup() {
		add_action('mounthood_action_shortcodes_list', 		'mounthood_sc_section_reg_shortcodes');
		if (function_exists('mounthood_exists_visual_composer') && mounthood_exists_visual_composer())
			add_action('mounthood_action_shortcodes_list_vc','mounthood_sc_section_reg_shortcodes_vc');
	}
}
...
if (!function_exists('mounthood_sc_section')) {	
	function mounthood_sc_section($atts, $content=null){	
		...
		$css .= ($color !== '' ? 'color:' . esc_attr($color) . ';' : '')
			...
			.($bg_image !== '' ? 'background-image:url(' . esc_url($bg_image) . ');'.(mounthood_param_is_on($bg_tile) ? 'background-repeat:repeat;' : 'background-repeat:no-repeat;background-size:cover;') : '')
			.(!mounthood_param_is_off($pan) ? 'position:relative;' : '')
			.($font_size != '' ? 'font-size:' . esc_attr(mounthood_prepare_css_value($font_size)) . '; line-height: 1.3em;' : '')
			.($font_weight != '' && !mounthood_param_is_inherit($font_weight) ? 'font-weight:' . esc_attr($font_weight) . ';' : '');
		$css_dim = mounthood_get_css_dimensions_from_values($width, $height);
		if ($bg_image == '' && $bg_color == '' && $bg_overlay==0 && $bg_texture==0 && mounthood_strlen($bg_texture)<2) $css .= $css_dim;
		
		$width  = mounthood_prepare_css_value($width);
		$height = mounthood_prepare_css_value($height);
	
		if ((!mounthood_param_is_off($scroll) || !mounthood_param_is_off($pan)) && empty($id)) $id = 'sc_section_'.str_replace('.', '', mt_rand());
	
		if (!mounthood_param_is_off($scroll)) mounthood_enqueue_slider();
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				...
				. '<div class="sc_section_inner'
					. (mounthood_param_is_on($scroll) && !mounthood_param_is_off($scroll_controls) ? ' sc_scroll_controls sc_scroll_controls_'.esc_attr($scroll_dir).' sc_scroll_controls_type_'.esc_attr($scroll_controls) : '')
					. '">'
					. ($bg_image !== '' || $bg_color !== '' || $bg_overlay>0 || $bg_texture>0 || mounthood_strlen($bg_texture)>2
						? '<div class="sc_section_overlay'.($bg_texture>0 ? ' texture_bg_'.esc_attr($bg_texture) : '') . '"'
							. ' style="' . ($bg_overlay>0 ? 'background-color:rgba('.(int)$rgb['r'].','.(int)$rgb['g'].','.(int)$rgb['b'].','.min(1, max(0, $bg_overlay)).');' : '')
								...
								. '>'
								. '<div class="sc_section_content' . (mounthood_param_is_on($bg_padding) ? ' padding_on' : ' padding_off') . '"'
									. ' style="'.esc_attr($css_dim).'"'
									. '>'
						: '')
					. (!empty($subtitle) ? '<h6 class="sc_section_subtitle sc_item_subtitle">' . trim(mounthood_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($title) ? '<h2 class="sc_section_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_with_descr') . '">' . trim(mounthood_strmacros($title)) . '</h2>' : '')
					. (!empty($description) ? '<div class="sc_section_descr sc_item_descr">' . trim(mounthood_strmacros($description)) . '</div>' : '')
					...
					. (mounthood_param_is_on($pan) 
						...
						: '')
							...
							. (!empty($link) ? '<div class="sc_section_button sc_item_button">'.mounthood_do_shortcode('[trx_button link="'.esc_url($link).'" icon="icon-right"]'.esc_html($link_caption).'[/trx_button]').'</div>' : '')
					. (mounthood_param_is_on($pan) ? '</div>' : '')
					. (mounthood_param_is_on($scroll) 
						? '</div></div><div id="'.esc_attr($id).'_scroll_bar" class="sc_scroll_bar sc_scroll_bar_'.esc_attr($scroll_dir).' '.esc_attr($id).'_scroll_bar"></div></div>'
							. (!mounthood_param_is_off($scroll_controls) ? '<div class="sc_scroll_controls_wrap"><a class="sc_scroll_prev" href="#"></a><a class="sc_scroll_next" href="#"></a></div>' : '')
						: '')
					. ($bg_image !== '' || $bg_color !== '' || $bg_overlay > 0 || $bg_texture>0 || mounthood_strlen($bg_texture)>2 ? '</div></div>' : '')
					. '</div>'
				. '</div>';
		if (mounthood_param_is_on($dedicated)) {
			if (mounthood_storage_get('sc_section_dedicated')=='') {
				mounthood_storage_set('sc_section_dedicated', $output);
			}
			$output = '';
		}
		return apply_filters('mounthood_shortcode_output', $output, 'trx_section', $atts, $content);
	}
	mounthood_require_shortcode('trx_section', 'mounthood_sc_section');
}

if (!function_exists('mounthood_sc_block')) {	
	function mounthood_sc_block($atts, $content=null) {
        $atts = is_array($atts) ? $atts : array();
		$atts['class'] = (!empty($atts['class']) ? $atts['class'] . ' ' : '') . 'sc_section_block';
		return apply_filters('mounthood_shortcode_output', mounthood_sc_section($atts, $content), 'trx_block', $atts, $content);
	}
	mounthood_require_shortcode('trx_block', 'mounthood_sc_block');
}


/* Register shortcode in the internal SC Builder
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_section_reg_shortcodes' ) ) {
	//add_action('mounthood_action_shortcodes_list', 'mounthood_sc_section_reg_shortcodes');
	function mounthood_sc_section_reg_shortcodes() {
		...
		$sc = array(
			...
			"params" => array(
				...
				"columns" => array(
					...
					"value" => "none",
					"type" => "checklist",
					"options" => mounthood_get_sc_param('columns')
				), 
				"pan" => array(
				...
				"scroll_controls" => array(
					...
					"value" => "hide",
					"type" => "checklist",
					"options" => mounthood_get_sc_param('controls')
				),
				"scheme" => array(
		...
		mounthood_sc_map("trx_section", $sc);
	}
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_section_reg_shortcodes_vc' ) ) {
	//add_action('mounthood_action_shortcodes_list_vc', 'mounthood_sc_section_reg_shortcodes_vc');
	function mounthood_sc_section_reg_shortcodes_vc() {
		...
		$sc = array(
			...
			"params" => array(
				...
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(mounthood_get_sc_param('columns')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "title",
				...
				array(
					...
					"value" => array_flip(mounthood_get_sc_param('controls')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scheme",
		...
		class WPBakeryShortCode_Trx_Block extends MOUNTHOOD_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Section extends MOUNTHOOD_VC_ShortCodeCollection {}
	}
}
?>