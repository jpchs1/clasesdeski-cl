...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('mounthood_sc_search_theme_setup')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_sc_search_theme_setup' );
	function mounthood_sc_search_theme_setup() {
		add_action('mounthood_action_shortcodes_list', 		'mounthood_sc_search_reg_shortcodes');
		if (function_exists('mounthood_exists_visual_composer') && mounthood_exists_visual_composer())
			add_action('mounthood_action_shortcodes_list_vc','mounthood_sc_search_reg_shortcodes_vc');
	}
}
...
if (!function_exists('mounthood_sc_search')) {	
	function mounthood_sc_search($atts, $content=null){	
		...
		if ($style == 'fullscreen') {
			...
			if (empty($ajax)) $ajax = mounthood_get_theme_option('use_ajax_search');
			if (empty($state)) $state = "closed";
		} else if ($style == 'slide') {
			if (empty($ajax)) $ajax = mounthood_get_theme_option('use_ajax_search');
			if (empty($state)) $state = "closed";
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') . ' class="search_wrap search_style_'.esc_attr($style).' search_state_'.esc_attr($state)
						. (mounthood_param_is_on($ajax) ? ' search_ajax' : '')
					...
					. '>
						...
						. (mounthood_param_is_on($ajax) ? '<div class="search_results widget_area' . ($scheme && !mounthood_param_is_off($scheme) && !mounthood_param_is_inherit($scheme) ? ' scheme_'.esc_attr($scheme) : '') . '"><a class="search_results_close icon-cancel"></a><div class="search_results_content"></div></div>' : '')
					. '</div>';
		return apply_filters('mounthood_shortcode_output', $output, 'trx_search', $atts, $content);
	}
	mounthood_require_shortcode('trx_search', 'mounthood_sc_search');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_search_reg_shortcodes' ) ) {
	//add_action('mounthood_action_shortcodes_list', 'mounthood_sc_search_reg_shortcodes');
	function mounthood_sc_search_reg_shortcodes() {
	
		mounthood_sc_map("trx_search", array(
			"title" => esc_html__("Search", 'trx_utils'),
			...
			"params" => array(
				"style" => array(
					...
					"value" => "regular",
					"options" => mounthood_get_list_search_styles(),
					"type" => "checklist"
				),
				"state" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_search_reg_shortcodes_vc' ) ) {
	//add_action('mounthood_action_shortcodes_list_vc', 'mounthood_sc_search_reg_shortcodes_vc');
	function mounthood_sc_search_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"value" => mounthood_get_list_search_styles(),
					"type" => "dropdown"
				),
				array(
					"param_name" => "state",
		...
		class WPBakeryShortCode_Trx_Search extends MOUNTHOOD_VC_ShortCodeSingle {}
	}
}
?>