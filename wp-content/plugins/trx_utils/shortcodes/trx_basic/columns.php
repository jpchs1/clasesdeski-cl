...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('mounthood_sc_columns_theme_setup')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_sc_columns_theme_setup' );
	function mounthood_sc_columns_theme_setup() {
		add_action('mounthood_action_shortcodes_list', 		'mounthood_sc_columns_reg_shortcodes');
		if (function_exists('mounthood_exists_visual_composer') && mounthood_exists_visual_composer())
			add_action('mounthood_action_shortcodes_list_vc','mounthood_sc_columns_reg_shortcodes_vc');
	}
}
...
if (!function_exists('mounthood_sc_columns')) {	
	function mounthood_sc_columns($atts, $content=null){	
		...
		mounthood_storage_set('sc_columns_data', array(
			'counter' => 1,
			'equal_selector' => '',
            'after_span2' => false,
            'after_span3' => false,
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="columns_wrap sc_columns'
					. ' columns_' . (mounthood_param_is_on($fluid) ? 'fluid' : 'nofluid') 
					. (!empty($margins) && mounthood_param_is_off($margins) ? ' no_margins' : '') 
					. ' sc_columns_count_' . esc_attr($count)
					. (!empty($class) ? ' '.esc_attr($class) : '') 
				. '"'
				. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
				. (!mounthood_param_is_off($equalheight) ? ' data-equal-height="'.esc_attr(mounthood_storage_get_array('sc_columns_data', 'equal_selector')).'"' : '')
				. (!mounthood_param_is_off($animation) ? ' data-animation="'.esc_attr(mounthood_get_animation_classes($animation)).'"' : '')
				. '>'
					. trim($content)
				. '</div>';
		return apply_filters('mounthood_shortcode_output', $output, 'trx_columns', $atts, $content);
	}
	mounthood_require_shortcode('trx_columns', 'mounthood_sc_columns');
}


if (!function_exists('mounthood_sc_column_item')) {	
	function mounthood_sc_column_item($atts, $content=null) {
		if (mounthood_in_shortcode_blogger()) return '';
		extract(mounthood_html_decode(shortcode_atts( array(
			// Individual params
			"span" => "1",
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') . ' class="column-'.($span > 1 ? esc_attr($span) : 1).'_'.esc_attr(mounthood_storage_get_array('sc_columns_data', 'count')).' sc_column_item sc_column_item_'.esc_attr(mounthood_storage_get_array('sc_columns_data', 'counter')) 
					. (!empty($class) ? ' '.esc_attr($class) : '')
					. (mounthood_storage_get_array('sc_columns_data', 'counter') % 2 == 1 ? ' odd' : ' even') 
					. (mounthood_storage_get_array('sc_columns_data', 'counter') == 1 ? ' first' : '') 
					. ($span > 1 ? ' span_'.esc_attr($span) : '') 
					. (mounthood_storage_get_array('sc_columns_data', 'after_span2') ? ' after_span_2' : '') 
					. (mounthood_storage_get_array('sc_columns_data', 'after_span3') ? ' after_span_3' : '') 
					. (mounthood_storage_get_array('sc_columns_data', 'after_span4') ? ' after_span_4' : '') 
					. '"'
					. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
					. (!mounthood_param_is_off($animation) ? ' data-animation="'.esc_attr(mounthood_get_animation_classes($animation)).'"' : '')
					. '>'
					...
					. '</div>';
		mounthood_storage_inc_array('sc_columns_data', 'counter', $span);
		mounthood_storage_set_array('sc_columns_data', 'after_span2', $span==2);
		mounthood_storage_set_array('sc_columns_data', 'after_span3', $span==3);
		mounthood_storage_set_array('sc_columns_data', 'after_span4', $span==4);
		mounthood_storage_set_array('sc_columns_data', 'equal_selector', $bg_color!=='' || $bg_image !== '' ? '.sc_column_item_inner' : '.sc_column_item');
		return apply_filters('mounthood_shortcode_output', $output, 'trx_column_item', $atts, $content);
	}
	mounthood_require_shortcode('trx_column_item', 'mounthood_sc_column_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_columns_reg_shortcodes' ) ) {
	//add_action('mounthood_action_shortcodes_list', 'mounthood_sc_columns_reg_shortcodes');
	function mounthood_sc_columns_reg_shortcodes() {
	
		mounthood_sc_map("trx_columns", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_columns_reg_shortcodes_vc' ) ) {
	//add_action('mounthood_action_shortcodes_list_vc', 'mounthood_sc_columns_reg_shortcodes_vc');
	function mounthood_sc_columns_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Columns extends MOUNTHOOD_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Column_Item extends MOUNTHOOD_VC_ShortCodeCollection {}
	}
}
?>