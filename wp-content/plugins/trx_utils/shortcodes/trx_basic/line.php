...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('mounthood_sc_line_theme_setup')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_sc_line_theme_setup' );
	function mounthood_sc_line_theme_setup() {
		add_action('mounthood_action_shortcodes_list', 		'mounthood_sc_line_reg_shortcodes');
		if (function_exists('mounthood_exists_visual_composer') && mounthood_exists_visual_composer())
			add_action('mounthood_action_shortcodes_list_vc','mounthood_sc_line_reg_shortcodes_vc');
	}
}
...
if (!function_exists('mounthood_sc_line')) {	
	function mounthood_sc_line($atts, $content=null){	
		...
		if ($style=='image' && !empty($image)) {
			...
			$attr = mounthood_getimagesize($image);
		...
		$css .= mounthood_get_css_dimensions_from_values($width, $block_height)
			. ($style=='image' && !empty($image)
				? ( 'background-image: url(' . esc_url($image) . ');'
					. (mounthood_param_is_on($repeat) ? 'background-repeat: repeat-x;' : '')
					)
				: ( ($height !='' ? 'border-'.esc_attr($border_pos).'-width:' . esc_attr(mounthood_prepare_css_value($height)) . ';' : '')
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id) . '"' : '') 
				...
				. '</div>';
		return apply_filters('mounthood_shortcode_output', $output, 'trx_line', $atts, $content);
	}
	mounthood_require_shortcode('trx_line', 'mounthood_sc_line');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_line_reg_shortcodes' ) ) {
	//add_action('mounthood_action_shortcodes_list', 'mounthood_sc_line_reg_shortcodes');
	function mounthood_sc_line_reg_shortcodes() {
	
		mounthood_sc_map("trx_line", array(
			...
			"params" => array(
				"style" => array(
					...
					"value" => "solid",
					"dir" => "horizontal",
					"options" => mounthood_get_list_line_styles(),
					"type" => "checklist"
				),
				"image" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_line_reg_shortcodes_vc' ) ) {
	//add_action('mounthood_action_shortcodes_list_vc', 'mounthood_sc_line_reg_shortcodes_vc');
	function mounthood_sc_line_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"std" => "solid",
					"value" => array_flip(mounthood_get_list_line_styles()),
					"type" => "dropdown"
				),
				array(
					"param_name" => "image",
				...
				array(
					...
					"class" => "",
					"std" => "center center",
					"value" => array_flip(mounthood_get_list_bg_image_positions()),
					"type" => "dropdown"
				),
		...
		class WPBakeryShortCode_Trx_Line extends MOUNTHOOD_VC_ShortCodeSingle {}
	}
}
?>