...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('mounthood_sc_video_theme_setup')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_sc_video_theme_setup' );
	function mounthood_sc_video_theme_setup() {
		add_action('mounthood_action_shortcodes_list', 		'mounthood_sc_video_reg_shortcodes');
		if (function_exists('mounthood_exists_visual_composer') && mounthood_exists_visual_composer())
			add_action('mounthood_action_shortcodes_list_vc','mounthood_sc_video_reg_shortcodes_vc');
	}
}
...
if (!function_exists('mounthood_sc_video')) {	
	function mounthood_sc_video($atts, $content = null) {
		...
		if (empty($height) && empty($width)) {
			$width='100%';
			if (mounthood_param_is_off(mounthood_get_custom_option('substitute_video'))) $height="400";
		}
		$ed = mounthood_substr($width, -1);
		...
		$css_bg = mounthood_get_css_paddings_from_values($bg_top, $bg_right, $bg_bottom, $bg_left);
	
		if ($src=='' && $url=='' && isset($atts[0])) {
			$src = $atts[0];
		}
		$url = $src!='' ? $src : $url;
		if ($image!='' && mounthood_param_is_off($image))
			$image = '';
		else {
			if (mounthood_param_is_on($autoplay) && is_singular() && !mounthood_storage_get('blog_streampage'))
				$image = '';
			else {
				if ($image > 0) {
				...
				if ($bg_image) {
					...
					if (!is_single() || !empty($image)) $image = mounthood_get_resized_image_url(empty($image) ? get_the_ID() : $image, $thumb_sizes['w'], $thumb_sizes['h'], null, false, false, false);
				} else
					if (!is_single() || !empty($image)) $image = mounthood_get_resized_image_url(empty($image) ? get_the_ID() : $image, $ed!='%' ? $width : null, $height);
				if (empty($image) && (!is_singular() || mounthood_storage_get('blog_streampage')))	// || mounthood_param_is_off($autoplay)))
					$image = mounthood_get_video_cover_image($url);
			}
		}
		if ($bg_image > 0) {
		...
		$url = mounthood_get_video_player_url($src!='' ? $src : $url);
		...
		$video = '<video' . ($id ? ' id="' . esc_attr($id) . '"' : '') 
			...
			. (($image && mounthood_param_is_on(mounthood_get_custom_option('substitute_video'))) || (mounthood_param_is_on($autoplay) && is_singular() && !mounthood_storage_get('blog_streampage')) ? ' autoplay="autoplay"' : '') 
			. ' controls="controls" loop="loop"'
			. '>'
			. '</video>';
		if (mounthood_param_is_off(mounthood_get_custom_option('substitute_video'))) {
			if (mounthood_param_is_on($frame)) $video = mounthood_get_video_frame($video, $image, $css, $css_bg);
		} else {
		...
		return apply_filters('mounthood_shortcode_output', $video, 'trx_video', $atts, $content);
	}
	mounthood_require_shortcode("trx_video", "mounthood_sc_video");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_video_reg_shortcodes' ) ) {
	//add_action('mounthood_action_shortcodes_list', 'mounthood_sc_video_reg_shortcodes');
	function mounthood_sc_video_reg_shortcodes() {
	
		mounthood_sc_map("trx_video", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_video_reg_shortcodes_vc' ) ) {
	//add_action('mounthood_action_shortcodes_list_vc', 'mounthood_sc_video_reg_shortcodes_vc');
	function mounthood_sc_video_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Video extends MOUNTHOOD_VC_ShortCodeSingle {}
	}
}
?>