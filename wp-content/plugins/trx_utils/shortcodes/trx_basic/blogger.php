...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('mounthood_sc_blogger_theme_setup')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_sc_blogger_theme_setup' );
	function mounthood_sc_blogger_theme_setup() {
		add_action('mounthood_action_shortcodes_list', 		'mounthood_sc_blogger_reg_shortcodes');
		if (function_exists('mounthood_exists_visual_composer') && mounthood_exists_visual_composer())
			add_action('mounthood_action_shortcodes_list_vc','mounthood_sc_blogger_reg_shortcodes_vc');
	}
}
...
*/
mounthood_storage_set('sc_blogger_busy', false);

if (!function_exists('mounthood_sc_blogger')) {	
	function mounthood_sc_blogger($atts, $content=null){	
		if (mounthood_in_shortcode_blogger(true)) return '';
		extract(mounthood_html_decode(shortcode_atts(array(
			...
			"location" => "default",
			"dir" => "horizontal",
			"hover" => mounthood_get_theme_option('hover_style'),
			"hover_dir" => mounthood_get_theme_option('hover_dir'),
			"scroll" => "no",
			"controls" => "no",
			"rating" => "no",
			"info" => "yes",
		...
		mounthood_storage_set('sc_blogger_busy', true);
		mounthood_storage_set('sc_blogger_counter', 0);
		...
		if ($descr == '') $descr = mounthood_get_custom_option('post_excerpt_maxlength'.($columns > 1 ? '_masonry' : ''));
	
		if (!mounthood_param_is_off($scroll)) {
			mounthood_enqueue_slider();
			if (empty($id)) $id = 'sc_blogger_'.str_replace('.', '', mt_rand());
		}
		
		$class = apply_filters('mounthood_filter_blog_class',
					'sc_blogger'
					. ' layout_'.esc_attr($style)
					. ' template_'.esc_attr(mounthood_get_template_name($style))
					. (!empty($class) ? ' '.esc_attr($class) : '')
					. ' ' . esc_attr(mounthood_get_template_property($style, 'container_classes'))
					. ' sc_blogger_' . ($dir=='vertical' ? 'vertical' : 'horizontal')
					. (mounthood_param_is_on($scroll) && mounthood_param_is_on($controls) ? ' sc_scroll_controls sc_scroll_controls_type_top sc_scroll_controls_'.esc_attr($dir) : '')
					. ($descr == 0 ? ' no_description' : '')
					. ($title_align!='' && $title_align!='none' ? ' title_' . esc_attr($title_align) : ''),
					array('style'=>$style, 'dir'=>$dir, 'descr'=>$descr)
		);
	
		$container = apply_filters('mounthood_filter_blog_container', mounthood_get_template_property($style, 'container'), array('style'=>$style, 'dir'=>$dir));
		...
		$container2 = apply_filters('mounthood_filter_blog_container2', mounthood_get_template_property($style, 'container2'), array('style'=>$style, 'dir'=>$dir));
		...
		$output = '<div'
			...
			. '>'
			. ($container_start)
			. (!empty($subtitle) ? '<h6 class="sc_blogger_subtitle sc_item_subtitle">' . trim(mounthood_strmacros($subtitle)) . '</h6>' : '')
			. (!empty($title) ? '<h2 class="sc_blogger_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_with_descr') . '">' . trim(mounthood_strmacros($title)) . '</h2>' : '')
			. (!empty($description) ? '<div class="sc_blogger_descr sc_item_descr">' . trim(mounthood_strmacros($description)) . '</div>' : '')
			. ($container2_start)
			. ($style=='list' ? '<ul class="sc_list sc_list_style_iconed">' : '')
			. ($dir=='horizontal' && $columns > 1 && mounthood_get_template_property($style, 'need_columns') ? '<div class="columns_wrap">' : '')
			. (mounthood_param_is_on($scroll) 
		...
		if (mounthood_get_template_property($style, 'need_isotope')) {
			if (!mounthood_param_is_off($filters))
				$output .= '<div class="isotope_filters"></div>';
			if ($columns<1) $columns = mounthood_substr($style, -1);
		...
		if (!mounthood_param_is_off($only)) $args = mounthood_query_add_filters($args, $only);
		...
		while ( $query->have_posts() ) { $query->the_post();
			...
			mounthood_storage_inc('sc_blogger_counter');
	
			$args = array(
				'layout' => $style,
				'show' => false,
				'number' => mounthood_storage_get('sc_blogger_counter'),
				'add_view_more' => false,
				...
				"readmore" => $readmore,
				"loadmore" => $loadmore,
				"reviews" => mounthood_param_is_on($rating),
				"dir" => $dir,
				"scroll" => mounthood_param_is_on($scroll),
				"info" => mounthood_param_is_on($info),
				...
				// Get post data
				'strip_teaser' => false,
				'content' => mounthood_get_template_property($style, 'need_content'),
				'terms_list' => !mounthood_param_is_off($filters) || mounthood_get_template_property($style, 'need_terms'),
				'filters' => mounthood_param_is_off($filters) ? '' : $filters,
				'hover' => $hover,
				'hover_dir' => $hover_dir
			);
		...
		if (!mounthood_param_is_off($filters)) {
			...
			if ($filters == 'categories') {			// Use categories as filter items
				...
				$portfolio_parent = $cat ? max(0, mounthood_get_parent_taxonomy_by_property($cat, 'show_filters', 'yes', true, $taxonomy)) : 0;
				$args2 = array(
					'type'			=> $post_type,
			...
			if ($filters_list) {
				mounthood_storage_concat('js_code', '
					jQuery("#'.esc_attr($id).' .isotope_filters").append("'.addslashes($filters_list).'");
				');
			}
		}
		$output	.= (mounthood_param_is_on($scroll) 
				? '</div></div><div id="'.esc_attr($id).'_scroll_bar" class="sc_scroll_bar sc_scroll_bar_'.esc_attr($dir).' '.esc_attr($id).'_scroll_bar"></div></div>'
					. (!mounthood_param_is_off($controls) ? '<div class="sc_scroll_controls_wrap"><a class="sc_scroll_prev" href="#"></a><a class="sc_scroll_next" href="#"></a></div>' : '')
				: '')
			. ($dir=='horizontal' && $columns > 1 && mounthood_get_template_property($style, 'need_columns') ? '</div>' :  '')
			. ($style == 'list' ? '</ul>' : '')
			. ($container2_end)
			. (!empty($link) 
				? '<div class="sc_blogger_button sc_item_button">'.do_shortcode('[trx_button link="'.esc_url($link).'" size="large" style="simple" icon="icon-right-small"]'.esc_html($link_caption).'[/trx_button]').'</div>' 				: '')
			. ($container_end)
			. '</div>';
		...
		return apply_filters('mounthood_shortcode_output', $output, 'trx_blogger', $atts, $content);
	}
	mounthood_require_shortcode('trx_blogger', 'mounthood_sc_blogger');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_blogger_reg_shortcodes' ) ) {
	//add_action('mounthood_action_shortcodes_list', 'mounthood_sc_blogger_reg_shortcodes');
	function mounthood_sc_blogger_reg_shortcodes() {
	
		mounthood_sc_map("trx_blogger", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"value" => "accordion",
					"type" => "select",
					"options" => mounthood_get_sc_param('blogger_styles')
				),
				"filters" => array(
					...
					"dir" => "horizontal",
					"type" => "checklist",
					"options" => mounthood_get_sc_param('filters')
				),
				"hover" => array(
					...
					"value" => "",
					"type" => "select",
					"options" => mounthood_get_sc_param('hovers')
				),
				"hover_dir" => array(
					...
					"value" => "left_to_right",
					"type" => "select",
					"options" => mounthood_get_sc_param('hovers_dir')
				),
				"dir" => array(
				...
				"post_type" => array(
					...
					"value" => "post",
					"type" => "select",
					"options" => mounthood_get_sc_param('posts_types')
				),
				"ids" => array(
				...
				"only" => array(
					...
					"value" => "no",
					"type" => "select",
					"options" => mounthood_get_sc_param('formats')
				),
				"scroll" => array(
				...
				"location" => array(
					...
					"value" => "default",
					"type" => "select",
					"options" => mounthood_get_sc_param('locations')
				),
				"rating" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_blogger_reg_shortcodes_vc' ) ) {
	//add_action('mounthood_action_shortcodes_list_vc', 'mounthood_sc_blogger_reg_shortcodes_vc');
	function mounthood_sc_blogger_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"std" => "accordion",
					"class" => "",
					"value" => array_flip(mounthood_get_sc_param('blogger_styles')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "filters",
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(mounthood_get_sc_param('filters')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "hover",
					...
					"class" => "",
					"value" => array_flip(mounthood_get_sc_param('hovers')),
					'dependency' => array(
						'element' => 'style',
				...
				array(
					...
					"class" => "",
					"value" => array_flip(mounthood_get_sc_param('hovers_dir')),
					'dependency' => array(
						'element' => 'style',
						'value' => array('portfolio_2','portfolio_3','portfolio_4','grid_2','grid_3','grid_4','square_2','square_3','square_4','short_2','short_3','short_4','colored_2','colored_3','colored_4')
					),
				...
				array(
					...
					"value" => array_flip(mounthood_get_sc_param('locations')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "dir",
				...
				array(
					...
					"class" => "",
					"value" => array_flip(mounthood_get_sc_param('posts_types')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "ids",
				...
				array(
					...
					"class" => "",
					"group" => esc_html__('Query', 'trx_utils'),
					"value" => array_flip(mounthood_get_sc_param('formats')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scroll",
		...
		class WPBakeryShortCode_Trx_Blogger extends MOUNTHOOD_VC_ShortCodeSingle {}
	}
}
?>