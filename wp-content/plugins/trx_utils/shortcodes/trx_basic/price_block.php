...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('mounthood_sc_price_block_theme_setup')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_sc_price_block_theme_setup' );
	function mounthood_sc_price_block_theme_setup() {
		add_action('mounthood_action_shortcodes_list', 		'mounthood_sc_price_block_reg_shortcodes');
		if (function_exists('mounthood_exists_visual_composer') && mounthood_exists_visual_composer())
			add_action('mounthood_action_shortcodes_list_vc','mounthood_sc_price_block_reg_shortcodes_vc');
	}
}
...
if (!function_exists('mounthood_sc_price_block')) {	
	function mounthood_sc_price_block($atts, $content=null){	
		...
		$content = do_shortcode(mounthood_sc_clear_around($content));
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
			...
			. '</div>';
		return apply_filters('mounthood_shortcode_output', $output, 'trx_price_block', $atts, $content);
	}
	mounthood_require_shortcode('trx_price_block', 'mounthood_sc_price_block');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_price_block_reg_shortcodes' ) ) {
	//add_action('mounthood_action_shortcodes_list', 'mounthood_sc_price_block_reg_shortcodes');
	function mounthood_sc_price_block_reg_shortcodes() {
	
		mounthood_sc_map("trx_price_block", array(
			...
			"params" => array(
				"style" => array(
					...
					"value" => 1,
					"options" => mounthood_get_list_styles(1, 1),
					"type" => "checklist"
				),
				"title" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'mounthood_sc_price_block_reg_shortcodes_vc' ) ) {
	//add_action('mounthood_action_shortcodes_list_vc', 'mounthood_sc_price_block_reg_shortcodes_vc');
	function mounthood_sc_price_block_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"std" => 1,
					"value" => array_flip(mounthood_get_list_styles(1, 1)),
					"type" => "dropdown"
				),
				array(
					"param_name" => "title",
		...
		class WPBakeryShortCode_Trx_PriceBlock extends MOUNTHOOD_VC_ShortCodeSingle {}
	}
}
?>