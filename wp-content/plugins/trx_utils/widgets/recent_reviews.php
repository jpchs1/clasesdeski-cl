...
// Theme init
if (!function_exists('mounthood_widget_recent_reviews_theme_setup')) {
	add_action( 'mounthood_action_before_init_theme', 'mounthood_widget_recent_reviews_theme_setup', 1 );
	function mounthood_widget_recent_reviews_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('mounthood_exists_visual_composer') && mounthood_exists_visual_composer())
			add_action('mounthood_action_shortcodes_list_vc','mounthood_widget_recent_reviews_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('mounthood_widget_recent_reviews_load')) {
	add_action( 'widgets_init', 'mounthood_widget_recent_reviews_load' );
	function mounthood_widget_recent_reviews_load() {
		register_widget('mounthood_widget_recent_reviews');
	}
}

// Widget Class
class mounthood_widget_recent_reviews extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_reviews', 'description' => esc_html__('Recent reviews', 'trx_utils'));
		parent::__construct( 'mounthood_widget_recent_reviews', esc_html__('Mounthood - Recent reviews', 'trx_utils'), $widget_ops );

		// Add thumb sizes into list
		mounthood_add_thumb_sizes( array( 'layout' => 'widgets', 'w' => 75, 'h' => 75, 'title'=>esc_html__('Widgets', 'trx_utils') ) );
	}
	...
	function form($instance) {
		...
		$categories = mounthood_get_list_terms(false, mounthood_get_taxonomy_categories_by_post_type($post_type));
		?>
...
*/
if ( !function_exists( 'mounthood_sc_widget_recent_reviews' ) ) {
	function mounthood_sc_widget_recent_reviews($atts, $content=null){	
		...
		extract($atts);
		$type = 'mounthood_widget_recent_reviews';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_recent_reviews' 
								. (mounthood_exists_visual_composer() ? ' vc_widget_recent_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, mounthood_prepare_widgets_args(mounthood_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_reviews', 'widget_recent_reviews') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('mounthood_shortcode_output', $output, 'trx_widget_recent_reviews', $atts, $content);
	}
	mounthood_require_shortcode("trx_widget_recent_reviews", "mounthood_sc_widget_recent_reviews");
}


// Add [trx_widget_recent_reviews] in the VC shortcodes list
if (!function_exists('mounthood_widget_recent_reviews_reg_shortcodes_vc')) {
	function mounthood_widget_recent_reviews_reg_shortcodes_vc() {
		
		$posts_types = mounthood_get_list_posts_types(false);
		$categories = mounthood_get_list_terms(false, mounthood_get_taxonomy_categories_by_post_type('post'));
		...
		vc_map( array(
				...
				"params" => array(
					...
					array(
						...
						"class" => "",
						"value" => array_flip(mounthood_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $categories)),
						"type" => "dropdown"
					),
					array(
