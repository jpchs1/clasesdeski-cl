...
getDisablePreviewCheckbox));if(frmViewsEditorInfo.includeCopyOption)children.push(dataProcessor.placeholder("copy",getCopyViewCheckbox));children.push(getViewKeyInput(),getFormSelectWrapper());return collapsible(div({text:__("Advanced Settings","formidable-views")}),div({children:children}),{open:!isCalendarType()&&!isGridType()&&!isTableType()})}function getFormSelectWrapper(){return div({margin:"10px 0 0",children:[getFormSelectLabel(),getFormSelect()]})}function getLimitPlaceholder(){var limitPlaceholder=
dataProcessor.placeholder("limit",getViewLimitInput);limitPlaceholder.classList.add("frm_half");return limitPlaceholder}function getTooltipTrigger(text,placement){var output=span({class:["frm_help","frm_icon_font","frm_tooltip_icon"]});output.setAttribute("title",text);output.style.position="relative";output.style.top="3px";output.style.left="4px";if("undefined"!==typeof placement)output.setAttribute("data-placement",placement);return output}function getPageSizePlaceholder(){var pageSizePlaceholder=
dataProcessor.placeholder("pageSize",getPageSizeInput);pageSizePlaceholder.classList.add("frm_half");return pageSizePlaceholder}function getCalendarSettings(){return collapsible(div({text:__("Calendar Settings","formidable-views")}),dataProcessor.placeholder("fieldOptions",function(){return div({children:[div({children:[tag("label",{text:__("Event Date","formidable-views")}),select(getEventDateOptions(dataProcessor.data.dateFieldId),{id:"frm_date_field_id",onchange:function(){updateDataProcessor("dateFieldId",
this.value)}})]}),div({margin:"10px 0 0",children:[tag("label",{text:__("End Date or Day Count","formidable-views")}),select(getEndDateOrDayCountOptions(dataProcessor.data.edateFieldId),{id:"frm_edate_field_id",onchange:function(){updateDataProcessor("edateFieldId",this.value)}})]}),div({class:"frm_grid_container",children:[getRepeatField(),div({margin:"10px 0 0",class:"frm_half",children:[getEndRepeatLabel(),getEndRepeatSelect()]})]})]})}),{open:true})}function getRepeatField(){var output=div({margin:"10px 0 0",
class:"frm_half",children:[tag("label",{text:__("Repeat","formidable-views")}),select(getEventRepeatOptions(dataProcessor.data.repeatEventFieldId),{id:"frm_repeat_event_field_id",onchange:function(){updateDataProcessor("repeatEventFieldId",this.value)}})]});appendTooltipTriggerAferInputLabel(output,getRepeatFieldTooltipText());return output}function getRepeatFieldTooltipText(){return __("Select a form field that contains values like 1 week, 2 weeks, 1 year, etc. This will set the repeat period for each event.",
